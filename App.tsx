...
const App: React.FC = () => {
  ...
  const fetchAppContent = async (userId: string) => {
    try {
      // 1. Fetch all registered users (public profiles) for social features
      const { data: usersData } = await supabase.from('users').select('*');
      if (usersData) setRegisteredUsers(usersData as User[]);

      // 2. Fetch User Tasks
      const { data: tasksData } = await supabase.from('tasks').select('*').eq('user_id', userId);
      if (tasksData) {
        // Map snake_case DB to camelCase Interface if needed, or rely on JS flexibility if types match enough. 
        // Our SQL uses snake_case keys (user_id, scheduled_date). Interface uses camelCase (userId, scheduledDate).
        // We need to map them!
        ...
        // If no tasks found, maybe seed with INITIAL_TASKS if it's a new user?
        // For now, let's leave empty or maybe prompts user to create?
        // The requirement "Migrating" implies keeping functionality. 
        // If we lost localStorage tasks, we might want to offer migration? 
        // But for a fresh "Launch", starting clean is okay as long as saving works.
      }

      // 3. Fetch Gratitude Posts
      const { data: gratitudeData } = await supabase.from('gratitude_posts').select('*').order('created_at', { ascending: false });
      if (gratitudeData) {
        const mappedGratitude: GratitudePost[] = gratitudeData.map((g: any) => ({
          id: g.id,
          ...
          icon: g.icon,
          type: g.type,
          comments: [] // Comments would need another table or JSONB
        }));
        setGratitudePosts(mappedGratitude);
      }

      // 4. Fetch Messages
      const { data: messagesData } = await supabase.from('messages').select('*').or(`sender_id.eq.${userId},receiver_id.eq.${userId}`).order('created_at', { ascending: true });
      if (messagesData) {
  ...
  const sendMessage = async (receiverId: string, text: string) => {
    ...
    // Save to Supabase (map to snake_case)
    await supabase.from('messages').insert({
      id: undefined, // Let DB generate UUID or use explicit if needed. Since we use `m-` timestamp locally, we might want to let DB handle it? 
      // Wait, DB has UUID PK. Local uses string `m-timestamp`. DB will reject if we send `id: 'm-...'`.
      // Better to OMIT id for DB insert and let it gen GUID.
      // But if we want to read it back... we won't match local.
      // It's okay, fetchAppContent refreshes.
    ...
    if (receiverId === 'u-admin') {
      try {
        ...
        const response = await ai.models.generateContent({
          model: 'gemini-2.0-flash-exp', // Updated model to latest available or keep preview
          contents: [{ role: 'user', parts: [{ text: text }] }], // Correct generic format for newer SDKs if needed, or stick to simple text if valid
          config: {
        ...
        await supabase.from('messages').insert({
          sender_id: adminReply.senderId,
          receiver_id: adminReply.receiverId,
          text: adminReply.text,
          read: adminReply.read,
          created_at: new Date().toISOString()
        });
  ...
  const handleToggleTask = async (taskId: string) => {
    ...
    if (task) {
      ...
      // Save to Supabase with snake_case mapping
  ...
  const renderView = () => {
    ...
    switch (currentView) {
      ...
      case View.LOGIN: return <Login onGoToRegister={() => setCurrentView(View.REGISTER)} onGoToForgotPassword={() => setCurrentView(View.FORGOT_PASSWORD)} />;
      case View.REGISTER: return <Register onGoToLogin={() => setCurrentView(View.LOGIN)} />;
      ...
      case View.GRATITUDE: return <Gratitude
        setView={setCurrentView}
        posts={gratitudePosts}
        onLike={async (id) => {
          const isLiked = likedPosts.includes(id);
          // Local Update
          setLikedPosts(prev => isLiked ? prev.filter(x => x !== id) : [...prev, id]);
          setGratitudePosts(prev => prev.map(p => p.id === id ? { ...p, likes: p.likes + (isLiked ? -1 : 1) } : p));
          ...
          if (isLiked) {
            ...
            // Decrement count
            const { error: decError } = await supabase.rpc('decrement_likes', { post_id: id });
            if (decError) {
              // Fallback if RPC doesn't exist: fetch and update
              const currentPost = gratitudePosts.find(p => p.id === id);
              if (currentPost) await supabase.from('gratitude_posts').update({ likes: Math.max(0, currentPost.likes - 1) }).eq('id', id);
            }
          } else {
            // Like: Insert to post_likes
            await supabase.from('post_likes').insert({ user_id: user.id, post_id: id });
            // Increment count
            const { error: incError } = await supabase.rpc('increment_likes', { post_id: id });
            if (incError) {
              const currentPost = gratitudePosts.find(p => p.id === id);
              if (currentPost) await supabase.from('gratitude_posts').update({ likes: currentPost.likes + 1 }).eq('id', id);
            }
          }
        }}
        onComment={async (postId, text) => {
          const comment = { id: Date.now().toString(), author: user.name, text, time: 'Agora' };
          const updatedPosts = gratitudePosts.map(p => p.id === postId ? { ...p, comments: [...(p.comments || []), comment] } : p);
          setGratitudePosts(updatedPosts);

          // DB Update
          // We need to fetch current comments from DB or trust local? trusting local for Append.
          // Ideally: append to JSONB. 
          const post = updatedPosts.find(p => p.id === postId);
          if (post) {
            await supabase.from('gratitude_posts').update({ comments: post.comments }).eq('id', postId);
          }
        }}
        likedPosts={likedPosts}
        onUserClick={(id) => { setSelectedProfileId(id); setCurrentView(View.USER_PROFILE); }}
      />;
  ...
  const handleAddGratitude = async (content: string, anonymous: boolean) => {
    const newPost: GratitudePost = {
      id: `g-${Date.now()}`, // Temporary ID, DB will likely replace or we can use UUID lib if strict
    ...
    // Note: We might want to clear this temporary post and wait for DB fetch, OR keep it.
    // For now, keeping optimistic UI.
  ...
  const handleAttendEvent = (eventId: string) => {
    if (!attendedEvents.includes(eventId)) {
      ...
      if (event) {
        ...
        supabase.from('attended_events').insert({ user_id: user.id, event_id: eventId });
      }
    }
  };
